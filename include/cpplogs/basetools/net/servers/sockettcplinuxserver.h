...
namespace CppLogs
{
	class SocketTcpLinuxServer : public CppLogs::ServerBase
	{
	private:
		struct StEpSockFd
		{
		...
		int _port;
		std::vector<CppLogs::ServerBase::StCppLogsNetAddrInfo> _st_CppLogsNetAddrInfo;
		CppLogs::SocketTcpLinuxServer::StEpSockFd _StEpSockFd;
	...
	public:
		...
		CppLogs::Error::EnCppLogsNetError init() override
		{
			...
			if (::bind(_StEpSockFd.sockFd, (struct sockaddr*)&st_sockaddr_in, \
				...
				return CppLogs::Error::EnCppLogsNetError_BindFailed;
			}
			if (::listen(_StEpSockFd.sockFd, DEF_MAX_LISTEN_CNT) < 0) {
				return CppLogs::Error::EnCppLogsNetError_ListenFailed;
			}
			_StEpSockFd.epFd = ::epoll_create(DEF_MAX_CONNECT_CNT);
			if (_StEpSockFd.epFd < 0) {
				return CppLogs::Error::EnCppLogsNetError_CreateEpollFailed;
			}
		...
		CppLogs::Error::EnCppLogsNetError accept(CppLogs::ServerBase::StNetDataInfo** st_NetDataInfo) override
		{
			...
			*st_NetDataInfo = new CppLogs::ServerBase::StNetDataInfo[ready_num];
			for (int i = 0; i < ready_num; i++) {
				...
				if (st_epoll_events[i].data.fd == _StEpSockFd.sockFd) {
					...
					(*st_NetDataInfo)[i].net_event_type = CppLogs::ServerBase::NetEventType_Conn;
					...
					(*st_NetDataInfo)[i].net_event_type = CppLogs::ServerBase::NetEventType_Recv;
					if (size <= 0) {
						(*st_NetDataInfo)[i].net_event_type = CppLogs::ServerBase::NetEventType_DisConn;
						close(client_fd);
					}
				}
			}
		...
		CppLogs::Error::EnCppLogsNetError recv(char* data, int& size) override
		{
			return CppLogs::Error::EnCppLogsNetError_None;
		}
		CppLogs::Error::EnCppLogsNetError close(const int client_fd) override
		{
		...
		void free_struct(CppLogs::ServerBase::StNetDataInfo* st_NetDataInfo) override
		{
			if (st_NetDataInfo) {
		...
		std::vector<CppLogs::ServerBase::StCppLogsNetAddrInfo> connect_info() override
		{
